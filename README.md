...
Only `state_template`, `active_child_template` and the `data`/`target`/`action` of a command accept a template - same as the upstream [universal media player](https://www.home-assistant.io/integrations/universal/). `browse_media_entity` and the values under `attributes` are read as entity ids (`media_player.child`, `media_player.child|volume_level`, or several children joined by `-`); a Jinja2 block there resolves to nothing, and is now ignored with a warning in the log instead of silently breaking the entity.
